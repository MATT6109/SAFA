...
def gendata(
        ...
        data_out_path,
        label_out_path,
        num_person_in=5,  #pose estimation max person=5 
        num_person_out=1,   
        max_frame=6000):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='ProcessedAlphaPose Data Converter.')
    parser.add_argument(
        '--data_path', default='./data')
    parser.add_argument(
        '--out_folder', default='.data/My')
    arg = parser.parse_args()

    part = ['12A', '38A']
    for p in part:
        data_path = '{}/{}_out'.format(arg.data_path, p)
