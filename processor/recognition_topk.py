...
class FocalLoss(nn.Module):
    def __init__(self, alpha=1, gamma=2, reduction='mean'):
        """
        :param alpha: balance param
        :param gamma: focus param
    ...
    def forward(self, inputs, targets):
        """
        :param inputs: predict(logits)
        :param targets: true label
        :return: Focal Loss
        """
        probs = F.softmax(inputs, dim=1)
        # acc prob
        probs = probs.gather(dim=1, index=targets.view(-1, 1))
        probs = probs.view(-1)
...
class REC_Processor(Processor):
    ...
    def mill_loss(self, topk_score, labels, binary=False):
        """
    ...
    def train(self):
        ...
        for data, label in loader:
            ...
            # forward
            output, video_level_score= self.model(data)
            labels = label.repeat(output.size(0))
            loss = self.loss(output, labels)
            milloss = self.mill_loss(video_level_score, label)
            loss += milloss
            # backward
    ...
    def get_parser(add_help=False):
        ...
        parser = argparse.ArgumentParser(
            ...
            description='Spatial-Frequency Graph Convolution Network')
